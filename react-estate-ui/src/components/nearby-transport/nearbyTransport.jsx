...
const NearbyTransport = ({ latitude, longitude }) => {
  ...
  useEffect(() => {
    const fetchNearbyTransport = async () => {
      const overpassQuery = `
            [out:json];
            (
              node["public_transport"~"station|stop|platform"](around:1000, ${latitude}, ${longitude});
              node["railway"~"station|subway"](around:1000, ${latitude}, ${longitude});
              node["highway"="bus_stop"](around:1000, ${latitude}, ${longitude});
              node["aeroway"="aerodrome"](around:10000, ${latitude}, ${longitude});
              node["aeroway"="terminal"](around:10000, ${latitude}, ${longitude});
            );
            out body;
          `;
      ...
      try {
        ...
        // const transportPlaces = data.elements.map((element) => ({
        //   id: element.id,
        //   type: element.tags.public_transport || element.tags.railway || element.tags.highway,
  ...
  return (
    <div className='flex-column padding-sm' >
      ...
      <span style={{height:'45vh',overflow:'scroll'}}>
        {
